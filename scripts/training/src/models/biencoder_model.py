...
class BiencoderModel(nn.Module):
    ...
    def _encode(self, encoder: PreTrainedModel, input_dict: dict) -> Optional[torch.Tensor]:
        ...
        if self.args.pool_type == 'avg':
             embeds = last_hidden.sum(dim=1) / attention_mask.sum(dim=1)[..., None]
        elif self.args.pool_type == 'cls':
            embeds = last_hidden[:, 0]
        else:
            raise ValueError(f'Unknown pooling type: {self.args.pool_type}')
...
class BiencoderModelForInference(BiencoderModel):
    ...
    def build(cls, args: Arguments, **hf_kwargs):
        ...
        lm_q.embeddings.position_embeddings.requires_grad = False
        lm_p.embeddings.position_embeddings.requires_grad = False
