...
class RetrievalDataLoader:
    ...
    def _transform_func(self, examples: Dict[str, List]) -> Dict[str, List]:
        ...
        # NOTE here I think they sample the negatives:
        ...
        input_docs = [f"{t}. {d}" for t, d in zip(input_titles, input_docs)]
        # NOTE we concat instead of using text_pair because we do it this way in evaluation

        if self.args.query_prefix:
            queries = [f"{self.args.query_prefix} {q}" for q in examples['query']]

        if self.args.doc_prefix:
            input_docs = [f"{self.args.doc_prefix} {d}" for d in input_docs]
        ...
        # x = doc_batch_dict["input_ids"][0]
        # print(self.tokenizer.decode(x, skip_special_tokens=False))
        # '<s> Title</s></s> text...</s>' if using text_pair
